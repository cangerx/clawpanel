...
本文介绍如何用 Docker 部署 **Cpanl Web 版**，通过浏览器远程管理 OpenClaw。

> **Cpanl** 提供 Windows / macOS / Linux 桌面安装包；Docker 部署面向已有容器环境的 Web 管理场景。
...
Cpanl Web 版 = Vite 开发服务器 + `dev-api.js` 后端中间件，在容器内提供完整管理功能。
...
这样 Cpanl 和 Gateway 共享同一个 `openclaw-data` 卷，Cpanl 可以直接管理 Gateway。

---

## 方式三：自定义 Dockerfile

如果只需要 Cpanl Web（Gateway 在宿主机或其他地方运行）：
...
使用上面的 Compose 配置，Cpanl 和 Gateway 共享数据卷，Cpanl 自动通过本地回环连接 Gateway。

### 场景二：Gateway 在宿主机

如果 Gateway 运行在宿主机（不在 Docker 中），Cpanl 容器需要访问宿主机网络：
...
使用 `--network host` 后，容器共享宿主机网络，Cpanl 可以直接连接 `127.0.0.1:18789`。

### 场景三：Gateway 在远程服务器

修改 `openclaw.json` 中的 Gateway 端口配置，或在 Cpanl 面板中设置 Gateway 地址。
...
> **重要：** 必须配置 WebSocket 升级头，否则 Cpanl 无法通过 `/ws` 连接 Gateway。
...
1. **使用一体镜像（最简单）**：直接用预装了 OpenClaw + Gateway + Cpanl 的一体镜像，不需要在面板里点安装：
   ```bash
...
2. **Host 网络模式**：`--network host` 让 Cpanl 直接管理宿主机进程
