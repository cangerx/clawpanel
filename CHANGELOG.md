...
- **消息渠道插件一键更新** — 渠道卡片新增「更新插件」按钮，支持一键升级到 `@latest`

### 修复 (Fixes)

- **插件更新失败（已存在目录）** — Web 后端 `install_channel_plugin` 新增更新场景处理：安装前自动备份旧插件，失败自动回滚，避免 `plugin already exists` 导致更新中断
- **默认密码安全提醒交互** — 将顶部横幅改为右上角弹窗提醒，保留“前往安全设置”动作，减少遮挡主界面

### 改进 (Improvements)

- **默认密码提醒主题统一** — 安全提醒弹窗与按钮统一为绿色主题视觉风格
- **更新提示体验优化** — 关于页/全局更新提示增加后台更新状态信息与回退操作入口，Web/Tauri 行为趋于一致
